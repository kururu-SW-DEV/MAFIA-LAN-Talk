...
- **네트워크 보안**: 방장 전용 패킷(게임 종료·역할 통보 등)은 방장이 보낸 것만, 투표·밤 행동은 본인이 보낸 것만 받습니다. 참가자 이름은 처음 확인된 접속 주소(IP:포트)에 묶여서, 패킷에 남의 이름만 적어 보낸 요청은 무시하고, 직업 통보·마피아 비밀 대화·경찰 조사 결과 같은 비밀 패킷도 그 묶인 주소로만 보냅니다(같은 이름을 쓰는 접속자가 둘 이상이면 누가 진짜인지 알 수 없어 참가를 받지 않습니다). 사람 참가자는 최대 9명(AI 최소 1명, 총 10명)이며, 정원이 차면 참가 신청을 받지 않고 신청자에게 알립니다. 밤 행동은 그 사람의 실제 직업과 같은 것만 받습니다(시민이 경찰·의사 행동을 흉내 낼 수 없음). 투표 진행 알림에는 누구에게 투표했는지가 실리지 않습니다(화면뿐 아니라 패킷도 익명). 한계: 같은 PC에서 여러 개를 띄우는 등 접속 주소를 공유하는 경우나 UDP 패킷 자체를 위조하는 공격은 막지 못하며, 같은 네트워크 암호 키(`secret.key`)를 가진 참가자끼리의 신뢰를 전제로 합니다.
