...
- **네트워크 보안**: 방장 전용 패킷(게임 종료·역할 통보 등)은 방장이 보낸 것만, 투표·밤 행동은 본인이 보낸 것만 받습니다. 참가자 이름은 처음 확인된 접속 주소(IP:포트)에 묶여서, 패킷에 남의 이름만 적어 보낸 요청은 무시합니다. 밤 행동은 그 사람의 실제 직업과 같은 것만 받습니다(시민이 경찰·의사 행동을 흉내 낼 수 없음). 투표 진행 알림에는 누구에게 투표했는지가 실리지 않습니다(화면뿐 아니라 패킷도 익명). 한계: 같은 PC에서 여러 개를 띄우는 등 접속 주소를 공유하는 경우나 UDP 패킷 자체를 위조하는 공격은 막지 못하며, 같은 네트워크 암호 키(`secret.key`)를 가진 참가자끼리의 신뢰를 전제로 합니다.
