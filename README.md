...
- **마피아 공모**: 사람 마피아끼리는 밤에 자동으로 열리는 **마피아 전용 비밀방**에서 의논하고, AI 마피아는 팀 결정에 맞춰 살해 대상이 통일됩니다.
...
설정에 입력한 API 키는 실행 폴더의 `data/settings.json`에 저장되며 저장소에는 포함되지 않습니다(`.gitignore`).
...
| `mafia_ui.py` | 게임방 UI 믹스인 + 호스트/클라이언트 동기화 |
...
| `sounds/` | 연출 효과음 |
| `tests/` | 회귀 테스트 (`python tests/test_multiplayer_sync.py` 등) |
