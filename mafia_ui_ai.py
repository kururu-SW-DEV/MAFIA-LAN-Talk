...
class MafiaAIChatMixin:
    ...
    def _pump_ai_utt(self):
        q = self.__dict__.setdefault("_ai_utt_q", [])
        if not q:
            return
        now = time.time()
        wait = getattr(self, "_ai_utt_next", 0) - now
        if wait > 0:
            if not getattr(self, "_ai_utt_timer", None):
        ...
        while q:
            ...
            if self._show_ai_utt(name, color, text):
                ...
                break
        if q and not getattr(self, "_ai_utt_timer", None):
            self._ai_utt_timer = self.root.after(int(max(0.3, getattr(self, "_ai_utt_next", now) - time.time()) * 1000) + 30,
                                                 self._ai_utt_tick)

    def _ai_utt_tick(self):
        self._ai_utt_timer = None
        self._pump_ai_utt()
    ...
    def _show_ai_utt(self, name, color, text):
        ...
        if self.mafia_active and self.core.phase == Phase.DAY:
            ...
            if chain_cnt < 1 and random_mod.random() < 0.3:      # v1.107 — 연쇄 답장을 줄여 채팅이 몰아치지 않게
