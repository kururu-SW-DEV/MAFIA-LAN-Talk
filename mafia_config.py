...
# 협박 판정 — 마피아 게임에서 정상적으로 쓰는 말("철수 처형하자", "누가 죽인 걸까")은 넣지 않는다.
# (예전엔 "처형하"·"죽인"·"죽일"이 들어 있어 정상 발언이 협박으로 판정돼 AI가 몰표를 줬다.)
THREAT_KEYWORDS = ("죽여", "해치", "협박", "봉인")
VOTE_WINDOW = 15              # 투표 팝업 카운트다운(초)
AI_VOTE_LLM_FALLBACK_MS = 11000  # v1.87 — 낮 본투표: 이 안에 LLM이 답하지 않으면 무작위로 대신 찍는다
                                  # (밤 행동의 NIGHT_LLM_FALLBACK_MS와 같은 목적 — 느린/불통 LLM이
                                  # 매일 "유효표 없음"으로 게임을 멈추지 않게 한다)
